...
io.on('connection', (socket) => {
  ...
  socket.on('updateWidgetColor', color => {
    db.sequelize.query('UPDATE public.widgetcolor SET color = :color', {replacements: {color: color}}).then(() => {
      db.sequelize.query('SELECT color FROM public.widgetcolor').then(data => {
        io.emit('widgetColor', data[0][0].color)
        console.log(data)
      })
    });
  })
  socket.on('getWidgetColor', () => {
    db.sequelize.query('SELECT color FROM public.widgetcolor').then(data => {
      console.log(data)
      io.emit('widgetColor', data[0][0].color)
      console.log(data)
